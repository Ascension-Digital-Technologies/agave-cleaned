...
/// Collects IPv4 addresses assigned to local network interfaces.
///
/// Windows builds do not expose `nix::ifaddrs`. AF_XDP is Linux-only, and this helper is only
/// used to optimize local-destination egress fallback for the XDP QUIC socket. Returning an empty
/// set preserves normal QUIC behavior on Windows while keeping the streamer crate buildable.
#[cfg(windows)]
fn collect_local_ipv4_ips() -> io::Result<Vec<Ipv4Addr>> {
    Ok(Vec::new())
}
